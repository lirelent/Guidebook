...
import com.lireherz.guidebook.guidebook.drawing.VisualPage;
import com.lireherz.guidebook.guidebook.drawing.VisualText;
...
public class BookRendering implements IBookGraphics {
	...
	private static void drawFlexible (int x, int y, float tx, float ty, int w, int h, int tw, int th, float scale) {
		...
		float tsw = 1.0f / tw;
		float tsh = 1.0f / th;
		bufferbuilder.pos(x, y + hs, 0.0D).tex(tx * tsw, (ty + h) * tsh).endVertex();
		bufferbuilder.pos(x + ws, y + hs, 0.0D).tex((tx + w) * tsw, (ty + h) * tsh).endVertex();
		bufferbuilder.pos(x + ws, y, 0.0D).tex((tx + w) * tsw, ty * tsh).endVertex();
		bufferbuilder.pos(x, y, 0.0D).tex(tx * tsw, ty * tsh).endVertex();
		tessellator.draw();
	}
	...
	private static void drawCustomQuad (int dx, int dy, int dw, int dh, float sx, int sy, int sw, float sh, int tw, int th) {
		...
		bufferbuilder.pos(dx, dy + dh, 0.0D).tex(tsx, tsy + tsh).endVertex();
		bufferbuilder.pos(dx + dw, dy + dh, 0.0D).tex(tsx + tsw, tsy + tsh).endVertex();
		bufferbuilder.pos(dx + dw, dy, 0.0D).tex(tsx + tsw, tsy).endVertex();
		bufferbuilder.pos(dx, dy, 0.0D).tex(tsx, tsy).endVertex();
		tessellator.draw();
	}
	...
	public IAnimatedBookBackground createBackground (GuiGuidebook guiGuidebook) {
		...
		if (loc != null) {
			factory = BACKGROUND_FACTORY_MAP.get(loc);
		}
