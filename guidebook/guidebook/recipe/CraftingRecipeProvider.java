...
class CraftingRecipeProvider extends RecipeProvider {
	...
	private IRecipe getRecipesByOutput (@Nonnull ItemStack targetOutput, int recipeIndex) {
		return ForgeRegistries.RECIPES.getValuesCollection().stream().filter(r -> !r.isDynamic() && ItemStack.areItemsEqualIgnoreDurability(targetOutput, r.getRecipeOutput())
				/*&& ItemStack.areItemStackTagsEqual(targetOutput, r.getRecipeOutput())*/).skip(recipeIndex).findFirst().orElse(null);
	}
	...
	private RecipeProvider.ProvidedComponents provideCraftingRecipeComponents (@Nullable IRecipe recipe) {
		...
		for (int i = 0; i < ingredients.size(); ++i) {
			...
			if (matching.length == 0) {
				continue; // If the recipe area is blank, continue and ignore
			}
